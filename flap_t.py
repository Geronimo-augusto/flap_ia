...
def main(genomas, config, ai_jogando=False):
    global geracao
    geracao += 1
...
def menu(tela):
    ...
    def set_ai_jogando():
        ...
        ai_jogando= True
        rodar(caminho_config, ai_jogando)
        return False
    ...
    while rodando:
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                pygame.quit()
                quit()
            rodando = botao1.check_click(evento)
            rodando = botao2.check_click(evento)
            rodando = botao3.check_click(evento)
            rodando = botao4.check_click(evento)
...
def rodar(caminho_config, ai_jogando):
