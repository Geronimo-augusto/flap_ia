...
def menu(tela):
    ...
    botao_iniciar = Button("Iniciar Jogo", 150, 200, 200, 50, RED, GREEN, iniciar_jogo_manual)
    botao_ai = Button("Modo IA", 150, 300, 200, 50, RED, GREEN, iniciar_jogo_ia)
    ...
    while rodando:
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                pygame.quit()
                quit()
            if botao_iniciar.check_click(evento):
                iniciar_jogo_manual()
                rodando = False
            if botao_ai.check_click(evento):
                iniciar_jogo_ia()
                rodando = False
